...
`learnBusiness` 用来理解本地业务资料，而不是理解代码仓库。输入可以是一批 PDF、Word、PowerPoint、图片、扫描页、流程图、表格、截图和混合 Office 导出文件。
...
- 读取一个本地业务文档目录。
- 把文本、表格、图片、页面、幻灯片和 AI 生成的图片说明抽取成可追溯内容块。
...
`.learnBusiness/` 不应提交到 Git。配置文件不保存 API key，真实密钥通过环境变量或外部密钥管理提供。
...
报告里的关键结论和问答答案都要能引用一个或多个 `Chunk`。AI 生成的内容必须显式标记，方便下游区分原始资料和模型推断。
