...
- `ai`：provider、base_url、chat_model、vision_model、embedding_model。
- `safety`：redact_before_external_ai、dry_run_ai。
- `performance`：context_chunks、chunk_char_limit。

默认 provider 是 `mock`，默认不把 API key 写进配置文件。
...
`ai` 定义 `AiProvider` 抽象，覆盖图片理解、chunk 摘要、embedding 和问答。当前有确定性的 `MockAiProvider`，以及 OpenAI-compatible provider 的配置骨架；后者目前会先校验 API key，但实际 HTTP 调用还未实现。
...
`qa` 负责检索增强问答。它先用 `store.search_text` 按问题检索最多 5 个相关 chunk；没有命中时直接返回“未找到相关来源”，不调用 AI；有命中时把 chunk 作为上下文交给 provider，并返回去重后的来源路径。
...
- OpenAI-compatible provider 当前还没有实际 HTTP 执行，避免在骨架阶段意外出网。
...
AI provider 的核心扩展点是 `AiProvider` trait。新的 provider 只要实现图片理解、摘要、embedding 和问答接口，就可以接入现有 qa、describe-image 和未来 ingest 补全流程。OpenAI-compatible provider 已经保留 base_url、chat_model、vision_model 和 embedding_model 配置字段，适合后续接入兼容接口。
...
- 有界问答上下文：默认 context_chunks 为 5，先检索再问答。
...
- OpenAI-compatible provider 只有配置骨架，实际 HTTP 调用尚未实现。
- ask 默认使用 mock provider，还没有按 `app.toml` 动态选择真实 provider。
